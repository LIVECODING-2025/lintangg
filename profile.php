...
// Koneksi DB (pastikan ini sesuai dengan file function.php atau langsung isi manual)
...
// Masukkan data dalam database (jika ada form dikirim)
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $alamat = mysqli_real_escape_string($koneksi, $_POST['alamat'] ?? '');
    $notelfon = mysqli_real_escape_string($koneksi, $_POST['notelfon'] ?? '');
    $nama = htmlspecialchars($_POST['nama'] ?? '');
    $username = htmlspecialchars($_POST['username'] ?? '');

    $query = "UPDATE data_login 
              SET alamat='$alamat', notelfon='$notelfon', nama='$nama', username='$username' 
              WHERE id_user=$id_user";
    mysqli_query($koneksi, $query) or die("Query gagal: " . mysqli_error($koneksi));

    // Redirect agar tidak POST ulang jika refresh
    header("Location: profile.php");
    exit;
}

// Ambil data terbaru setelah login atau setelah update
$data = query("SELECT * FROM data_login WHERE id_user = $id_user");
if (!$data || count($data) === 0) {
    die("Data pengguna tidak ditemukan.");
}

$result = $data[0]; // Ambil data pertama
$nama = $result['nama'];
$username = $result['username'];
$alamat = $result['alamat'];
$notelfon = $result['notelfon'];
...
<head>
  ...
  .profile-dropdown .dropdown-content {
    ...
    @media (max-width: 576px) {
      .btn {
        width: 100%; /* Tombol jadi selebar layar di mobile */
        margin-left: 10px;
      }
    }
  </style>
...
<div class="dropdown profile-dropdown">
  ...
  <div class="row">
    ...
    <div class="col-md-3 col-lg-2 sidebar">
      ...
      <a href="profile.php" class="active" class="sidebar-link"><i class="bi bi-person me-2"></i>User Info</a>
    ...
    <form action="" method="POST">
      <div class="row">
        ...
        <div class="col-md-5 px-5">
          ...
          <div class="mb-3">
            ...
            <input type="text" name="notelfon" class="form-control" id="notelfon" placeholder="isi nomor telepon anda" value="<?= htmlspecialchars($result['notelfon'] ?? '') ?>">
          </div>
        </div>
        ...
        <div class="col-md-5 px-5">
          ...
          <div class="mb-3">
            ...
            <input type="text" name="alamat" class="form-control" id="alamat" placeholder="isi alamat lengkap anda" value="<?= htmlspecialchars($result['alamat'] ?? '') ?>">
          </div>
        </div>
      </div>
      ...
      <div class="row">
        <div class="col mt-4 mb-4">
          <button type="submit" class="btn px-5 py-2" style="background-color: #013220; margin-left: 420px; color: white;">
            Simpan Data
          </button>
        </div>
      </div>
